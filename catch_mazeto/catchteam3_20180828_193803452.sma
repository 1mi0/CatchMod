...
// #define cl_debug(%1) client_print(0,print_chat,%1);
...
public roundEnd()
{
	...
	// Who Won
	new teamAlive[2];
	teamAlive[g_swapped] = get_playersnum_ex(GetPlayers_ExcludeDead | GetPlayers_MatchTeam, "TERRORIST");
	teamAlive[1 - g_swapped] = get_playersnum_ex(GetPlayers_ExcludeDead | GetPlayers_MatchTeam, "CT");
	...
	if(g_win[g_game] == 3)
	{
		if (teamAlive[(g_swapped - (catchmod_get_fleers() - 1)) % 2])
		{
			g_score[g_game][(g_swapped - (catchmod_get_fleers() - 1)) % 2]++;
		}
		else 
		{
			g_score[g_game][(1 - (g_swapped - (catchmod_get_fleers() - 1))) % 2]++;
		}
	}
	...
	if(win > -1 && g_win[g_game] == 3)
	{
		...
		if(win == 2)
		{
			client_print_color(0, 0, "^x04[ ====== Match Draw ====== ]");
		}
		else
		{
			client_print_color(0, 0, "^x01[ ====== ^x04%s^x01 Wins Game ^x04%d^x01 ====== ]", g_teamName[win], g_game);
		}
		...
		for(new i = 1; i <= g_game; i++)
		{
			...
			if(wins[0] >= 2)
			{
				client_print_color(0, 0, "^x01[ ====== ^x04%s^x01 Wins ^x04Match^x01 ====== ]", g_teamName[0]);
				client_print_color(0, 0, "^x01[ ====== ^x04%s^x01 Wins ^x04Match^x01 ====== ]", g_teamName[0]);
				client_print_color(0, 0, "^x01[ ====== ^x04%s^x01 Wins ^x04Match^x01 ====== ]", g_teamName[0]);
				client_print(0, print_chat, "The Match will end in 15 seconds");
				set_task(15.0, "disableWarDelayed");
			}
			else if(wins[1] >= 2)
			{
				client_print_color(0, 0, "^x01[ ====== ^x04%s^x01 Wins ^x04Match^x01 ====== ]", g_teamName[1]);
				client_print_color(0, 0, "^x01[ ====== ^x04%s^x01 Wins ^x04Match^x01 ====== ]", g_teamName[1]);
				client_print_color(0, 0, "^x01[ ====== ^x04%s^x01 Wins ^x04Match^x01 ====== ]", g_teamName[1]);
				client_print(0, print_chat, "The Match will end in 15 seconds");
				set_task(15.0, "disableWarDelayed");
		...
		{
			client_print_color(0, 0, "^x01[ == ^x04 %s^x01  [^x04%d^x01] vs ^x04 %s^x01  [^x04%d^x01] == ]", g_teamName[0], g_score[g_game][0], g_teamName[1], g_score[g_game][1]);
...
public roundStart()
{
	...
	if(g_win[g_game] == 3)
	{
		client_print_color(0, 0, "[ == ^x03 %s^x01  [%d] vs ^x03 %s^x01  [%d] == ]", g_teamName[0], g_score[g_game][0], g_teamName[1], g_score[g_game][1]);
...
public rebuildHudMessages()
{
	...
	if(catchmod_get_fleers() - 1 == g_swapped)
	{
		len += format(textScore[len], 479-len, "Game %d [F%d:%d]   ", g_game, g_score[g_game][0], g_score[g_game][1]);
	}
	else
	{
