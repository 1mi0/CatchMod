...
public plugin_init()
{
	...
	amx_turbo_cvar = register_cvar("amx_turbo", "30");
	amx_direction_cvar = register_cvar("amx_direction", "0");
	...
	teamPluginId = find_plugin_byfile("catchteam.amxx");
	if(teamPluginId != INVALID_PLUGIN_ID)
	{
		teamSwapId = get_func_id("teamSwapCallback", teamPluginId);
	}
	// Channels
	register_forward(FM_AddToFullPack, "FM_AddToFullPack_Pre", 0);
	register_clcmd("say", "hook_say");
	register_clcmd("say_team", "hook_say");
	register_clcmd("/ch", "CmdChannel");
}

public callSwapCallback(swap)
{
	if(teamPluginId == INVALID_PLUGIN_ID)
		return;
	
	callfunc_begin_i(teamSwapId, teamPluginId);
	callfunc_push_int(swap);
	callfunc_end();
}
...
public new_round()
{
	...
	amx_turbo = get_pcvar_num(amx_turbo_cvar);
	amx_direction = get_pcvar_num(amx_direction_cvar);
}
...
public player_showteam(id)
{
	...
	if(training == 1)
	{
		...
		format(turbos,sizeof turbos - 1,"^n%sTurbo: [======|======] %d%^n^nChannel: %d",turbo[id-1][0] == 1 ? "+" : "-",turbo[id-1][1], UserChannel[id]);
...
public Touch_World(id, world) 
{
	if(is_user_alive(id)) 
	{
		if(caughtJump[id] && (newButton[id] & IN_JUMP) && !(get_entity_flags(id) & FL_ONGROUND)) 
		{
			...
			// jumpVeloc[id][0] *= -1.0;
			jumpVeloc[id][amx_direction] *= -1.0;
...
public hook_say(id) {
	...
	if(szCommand[1] == '/' && szCommand[2] == 'c' && szCommand[3] == 'h') {
		UserChannel[id] = str_to_num(szCommand[5]);
		player_showteam(id);
	}
	return PLUGIN_CONTINUE;
}
public CmdChannel(id) {
	if( !is_user_connected(id) )
		return PLUGIN_HANDLED;
	...
	UserChannel[id] = str_to_num(szCommand);
	player_showteam(id);
	
	return PLUGIN_HANDLED;
}
public FM_AddToFullPack_Pre(es, e, iEnt, id, hostflags, player, pSet) { 
	if(player && id != iEnt) {
		if(UserChannel[iEnt] != UserChannel[id]) {
