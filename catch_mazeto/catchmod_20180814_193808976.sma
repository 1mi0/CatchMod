...
public apply(id)
{
	...
	if(team[id-1] == 1)
	{
		...
		client_print(id,print_chat,"[ Mazeto.org ] You are now a CATCHER. You have to catch. Go,Go,Go...");
	}
	else
	{
		client_print(id,print_chat,"[ Mazeto.org ] You have now to FLEE,Take care...");
	}
...
public endround()
{
	if(enable && !blockround)
	{
		...
		if(other_win == 1)
		{
			...
			{
				if(is_user_connected(i))
				client_print(i,print_chat,"[ Mazeto.org ] Fleers won this round. +%d Point%s",punkte*SURVIVE,punkte*SURVIVE > 1 ? "e" : "");
			}
		}
		else
		{
			...
			{
				if(is_user_connected(i))
				client_print(i,print_chat,"[ Mazeto.org ] Catchers won this round!");
			}	
		}
		if(team_ct == 1)
		{
